...
const init = function () {
    ...
    wdioDone = false;
    require('child_process').exec('pkill -f selenium-standalone');
}
...
const isSauceRequired = function () {
    return obj.config.sauceLabs ? true : false;
}
...
module.exports = function (cf, cb) {
    ...
    if (isServerDefined()) {
        var app = express();
        var appPath = path.resolve(configFile, '..', require(configFile).basePath, obj.server.app);
        app.use('/', express.static(appPath));
        server = app.listen(obj.server.port, function() {
    ...
    if (isSauceRequired()) {
        sc = SauceConnect(obj.config.user, obj.config.key, obj.config.saucelabs, function connected() {
            //  sc connected
            browserReady = true;
            seleniumReady = true;
