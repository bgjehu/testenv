...
var TestRunner = require('../components/TestRunner');        //  the 'class' that actually run the test with config file
...
var runCustomTest = function(args) {
    ...
    //  run the custom test
    new TestRunner(args.paths, function done() {

        //  in done callback, display end message
        console.log('End custom test(s).');
        process.exit(0);
    });
}
